...
import { Teacher } from './TeacherFormModal';
...
export const DeleteTeacherModal: React.FC<DeleteTeacherModalProps> = ({
  ...
  const handleDelete = async () => {
    setLoading(true);
    const toastId = toast.loading('Deleting faculty profile...');
    ...
    try {
      ...
      if (res?.success) {
        toast.success('Faculty profile deleted successfully!', { id: toastId });
        onSuccess(teacher.id);
        onClose();
      } else {
        toast.error(res?.message || 'Failed to delete teacher', { id: toastId });
      }
    } catch (error: any) {
      toast.error(error.message || 'Error occurred while deleting teacher', {
        id: toastId,
      });
    } finally {
      setLoading(false);
        ...
        <div className="flex items-start gap-4">
          ...
          <div>
            ...
            <p className="mt-1 text-xs text-muted-foreground leading-relaxed">
              Are you sure you want to remove <span className="font-semibold text-foreground">"{teacher.name}"</span> ({teacher.designation})?
              This record will be archived from the public faculty directory.
            </p>
          </div>
        </div>
