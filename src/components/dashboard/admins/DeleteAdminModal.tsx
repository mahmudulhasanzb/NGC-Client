...
import { AdminUser } from './AdminFormModal';
...
export const DeleteAdminModal: React.FC<DeleteAdminModalProps> = ({
  ...
  const handleDelete = async () => {
    setLoading(true);
    const toastId = toast.loading('Revoking administrator access...');
    ...
    try {
      ...
      if (res?.success) {
        toast.success('Administrator access revoked successfully!', { id: toastId });
        onSuccess(admin.id);
        onClose();
      } else {
        toast.error(res?.message || 'Failed to delete administrator', { id: toastId });
      }
    } catch (error: any) {
      toast.error(error.message || 'Error occurred while revoking admin', {
        id: toastId,
      });
    } finally {
      setLoading(false);
        ...
        <div className="flex items-start gap-4">
          ...
          <div>
            ...
            <p className="mt-1 text-xs text-muted-foreground leading-relaxed">
              Are you sure you want to revoke administrative permissions for <span className="font-semibold text-foreground">"{admin.name}"</span> ({admin.email})?
              They will no longer be able to log in to the management dashboard.
            </p>
          </div>
        </div>
