...
export const PublicGalleryList: React.FC<PublicGalleryListProps> = ({
  ...
  return (
    <div className="min-h-screen bg-background">
      {/* Page Hero Header */}
      <section className="border-b border-border/80 bg-secondary/30 py-8 sm:py-10">
              ...
              {initialItems.map((item, index) => (
                ...
                >
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-5 text-white opacity-0 transition-opacity duration-300 group-hover:opacity-100">
                    {item.category && (
                      <span className="mb-1 inline-block rounded-md bg-white/20 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider backdrop-blur-xs">
                        {item.category}
                      </span>
                    )}
